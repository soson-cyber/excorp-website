...
- **모션/Glow 규칙 반전**: 기존 "Glow 금지"를 **해제**. 허용: 버튼 **글로우-블리드 헤일로**(`.btn::after`, Wope), **커서 spotlight**, 히어로 **파티클 캔버스·핫퍼플 블룸**. 모든 모션은 `prefers-reduced-motion` 정적 경로 필수.
- **글로우-라인 = 버튼 전용 (v3.1)**: 박스(콘솔·피처카드)의 회전 conic 보더는 **제거**(`.console-shell .edge, .feat .feat-edge{display:none}`). 대신 **버튼 테두리에 애니메이션 conic 글로우-라인** — `.btn::after` + `@property --ex-angle` + `conic-gradient` + `btnBorderSpin 4.5s linear`. → §6/§8의 "요소 박스 글로우 금지"는 **버튼 한정 글로우-라인은 허용**으로 갱신(박스/텍스트 글로우는 여전히 금지).
- **히어로 (v3.1)**: 동심원 오로라 → **product-led 다크 히어로 + 인터랙티브 EXLINK Live Console**(멀티캠 클릭→프로그램 피드, telemetry, pipeline, LIVE/REC). 배경은 **정적 CSS 퍼스펙티브 그리드**(`.hero-grid` + `::before`, `perspective()/rotateX()` + mask 페이드, Wope식) + `HeroStars` 캔버스 + 콘솔 상단 핫퍼플 radial 블룸. **three.js 완전 제거**. `components/home/Hero.tsx`.
- **네브바 = 투명 바 + 중앙 글래스 필 (v3.1)**: 스크롤 시 바 자체 투명(`.header--solid{background:transparent}`), 중앙 메뉴를 반투명 라운드 필이 감쌈(`.navbar` — `bg rgba(255,255,255,.05)`·`backdrop-filter blur(12px)`·`9999px`). 서브메뉴 셰브론(`.caret`/`.caret--open`), 넉넉한 간격(`.navlink padding 9px 20px`). ⚠ headless Chromium은 `backdrop-filter` 미렌더.
- **그레인 오버레이 (v3.1)**: `layout.tsx` SVG `feTurbulence` 필름그레인(`.grain-overlay`, fixed z-60).
- **홈 레이아웃 (v3.1)**: §01 `featgrid--wope`(3번째 카드 `full`), §03 파트너 `partner-bento`, **FAQ `<details class="faq-item">` 아코디언**, CTA `ctacard`, Numbers/Quote `section--glow`.
...
> **규칙 (⚠ §0 v3에서 갱신됨)**: ~~요소 박스 글로우 전면 금지~~ → 다크 전환 후 **버튼 헤일로 + 버튼 conic 글로우-라인은 허용**(§0). 단 **박스(카드/콘솔)·텍스트 글로우는 여전히 금지** — 박스 깊이는 그림자·보더로만, 박스의 회전 보더는 v3.1에서 제거됨.
...
### Don't
- ❌ **박스(카드/콘솔)·텍스트 글로우·네온 박스섀도우**. (단 §0 v3 갱신: **버튼 헤일로/conic 글로우-라인은 허용** — 버튼 한정.)
