...
- **알려진 편차(의도적)**: ① 다크 히어로 대형 제목 **글로우 생략**(규칙상 권장이나 브랜드 NO-glow 우선). ② Home·신규 컴포넌트 **리터럴 hex 사용**(스킬의 "zero hardcoded hex" 이상과 다름 — 단 전부 본 팔레트 값과 일치, 토큰화는 차기 과제). ③ 외부 크롤러/토큰추출 스크립트 미실행(사이트·토큰 기보유).
- **차기 정합 과제**: 리터럴 hex → 토큰 매핑, 섹션 헤더 정렬/패딩 리듬 통일, CTA 문구 통일, 스펙/비교표 컴포넌트화(+`<table>` 시맨틱), 한글 본문 행간 1.7 일괄.
